...
function patchTimestamp(trama) {
  ...
  if (ts < MIN || ts > MAX) {
    console.error(`  [ERROR PRUEBA] Fecha UNIX fuera de rango: ${ts}`);
    process.exit(1);
  }
...
function send(client, message, label) {
  return new Promise((resolve) => {
    ...
    client.send(buf, 0, buf.length, PORT, HOST, (err) => {
      ...
      else     console.log(`  [ENVIADO] ${label}`);
      resolve();
    });
  });
}
...
async function runTests() {
  ...
  console.log('\n==========================================================');
  console.log('  SUITE DE PRUEBAS — Suntech + Ruptela + Stress');
  console.log('==========================================================\n');
  ...
  await send(client, RUPTELA_FUEL(),    'Ruptela Combustible    — combustible Bluetooth');
  await sleep(300);

  await send(client, RUPTELA_TEMP(),    'Ruptela Temperatura    — temperatura y humedad Bluetooth');
  await sleep(300);

  await send(client, RUPTELA_SCAN(),    'Ruptela Escaneo    — OBD/CAN, 2 sub-tramas');
  await sleep(800);
  ...
  await send(client, tramaFijaDup, 'Ruptela duplicado #1 — debe guardarse    âœ“');
  await sleep(100);
  await send(client, tramaFijaDup, 'Ruptela duplicado #2 — debe ignorarse    âœ—');
  await sleep(100);
  await send(client, tramaFijaDup, 'Ruptela duplicado #3 — debe ignorarse    âœ—');
  await sleep(800);
  ...
  client.send(RAW_SUNTECH, 0, RAW_SUNTECH.length, PORT, HOST, (err) => {
    if (err) console.error(`  [ERROR] Suntech raw: ${err.message}`);
    else     console.log('  [ENVIADO] Trama raw sin JSON — fallback Suntech (28 campos)');
  });
  await sleep(800);
  ...
  for (let i = 0; i < total; i++) {
    ...
    client.send(buf, 0, buf.length, PORT, HOST, (err) => {
      ...
      if (sent % 100 === 0) {
        ...
        console.log(`  [CARGA] ${sent}/${total} enviados — ${elapsed}s transcurridos`);
      }
      if (sent === total) {
        const totalTime = ((Date.now() - startTime) / 1000).toFixed(2);
        console.log(`\n  [CARGA] Completado — ${total} tramas en ${totalTime}s`);
        console.log(`  [CARGA] Promedio   — ${(total / parseFloat(totalTime)).toFixed(0)} tramas/seg\n`);
...
function printSummary() {
  ...
  console.log('  | 864134057777777st  (Suntech raw)         |  1 doc |');
  console.log('  | 860369050167418ru  (Ruptela Normal+Dup)  |  1 doc |');
  console.log('  | [IMEI Fuel]ru      (Ruptela Combustible)        |  1 doc |');
  console.log('  | [IMEI Temp]ru      (Ruptela Temperatura)        |  1 doc |');
  console.log('  | [IMEI Scan]ru      (Ruptela Escaneo)        |  1 doc |');
  console.log('  +------------------------------------------+--------+');
